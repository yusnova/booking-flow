...
## BUG-001 — “first request fails” counter is shared across the whole server process

| Field | Value |
| --- | --- |
| **Severity** | High (multi-user / shared process) |
| **Priority** | P3 |
...
1. For postcode `BS1 4DJ`, the **first** lookup is designed to return HTTP **500**, and a **second** attempt should succeed.
...
## BUG-002 — After changing plasterboard handling, skip rules are not obvious until you revisit step 3
...
3. From the three plasterboard handling options, select **Dedicated plasterboard skip only**.
...
7. Change the plasterboard handling choice to **Less than 10% plasterboard (mixed load)**.
...
## BUG-003 — Postcode field and address list can feel out of sync after “Back”
